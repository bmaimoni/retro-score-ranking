...
async def test_anonimo_nick_livre_segue_sem_reivindicar(fake_pool):
    ...
    with patch("auth.repository.buscar_nick_claim", AsyncMock(return_value=None)), \
        ...
        await auth_svc.verificar_e_reivindicar_nick(fake_pool, "novato", user_id=None)
...
async def test_anonimo_nick_ja_reivindicado_bloqueia(fake_pool):
    ...
    with patch("auth.repository.buscar_nick_claim", AsyncMock(return_value=claim)):
        with pytest.raises(auth_svc.NickJaReivindicadoError):
            await auth_svc.verificar_e_reivindicar_nick(fake_pool, "campeao", user_id=None)
...
async def test_logado_nick_livre_reivindica(fake_pool):
    ...
    with patch("auth.repository.buscar_nick_claim", AsyncMock(return_value=None)), \
        ...
        await auth_svc.verificar_e_reivindicar_nick(fake_pool, "novato", user_id=user_id)

    criar_mock.assert_called_once_with(fake_pool, "novato", user_id)


@pytest.mark.asyncio
async def test_logado_nick_ja_e_seu_segue_normal(fake_pool):
    user_id = make_uuid()
    claim = {"id": make_uuid(), "nick_norm": "campeao", "user_id": user_id, "criado_em": "2026-01-01"}

    with patch("auth.repository.buscar_nick_claim", AsyncMock(return_value=claim)), \
         patch("auth.repository.criar_nick_claim") as criar_mock:
        # Não deve levantar exceção nenhuma
        await auth_svc.verificar_e_reivindicar_nick(fake_pool, "campeao", user_id=user_id)
...
async def test_logado_nick_de_outro_usuario_bloqueia(fake_pool):
    dono_id = make_uuid()
    outro_id = make_uuid()
    claim = {"id": make_uuid(), "nick_norm": "campeao", "user_id": dono_id, "criado_em": "2026-01-01"}

    with patch("auth.repository.buscar_nick_claim", AsyncMock(return_value=claim)):
        with pytest.raises(auth_svc.NickJaReivindicadoError):
            await auth_svc.verificar_e_reivindicar_nick(fake_pool, "campeao", user_id=outro_id)
