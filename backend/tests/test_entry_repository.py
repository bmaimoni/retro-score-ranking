"""
Testes de repositories/entry.py — funções novas de
...
async def test_vincular_retroativamente_conta_linhas_afetadas(fake_pool):
    ...
    resultado = await entry_repo.vincular_retroativamente(fake_pool, "novato", "u1")
...
@pytest.mark.asyncio
async def test_vincular_retroativamente_zero_entries_orfas(fake_pool):
...
async def test_marcar_pendente_identificacao_ambigua(fake_pool):
    ...
    resultado = await entry_repo.marcar_pendente_identificacao_ambigua(fake_pool, "veterano")
...
async def test_listar_feed_admin_expoe_pendente_motivo_sempre(fake_pool):
    ...
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool)
...
async def test_listar_feed_admin_status_pendentes_traz_contexto_de_ranking(fake_pool):
    ...
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, status="pendentes")
...
async def test_listar_feed_admin_status_todos_nao_paga_custo_do_contexto(fake_pool):
    ...
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, status="todos")
...
async def test_listar_feed_admin_status_pendentes_exclui_arquivado(fake_pool):
    """Achado (Fase 5): sem essa exclusão, uma entry
...
async def test_contar_feed_admin_tambem_arquiva_antes_de_contar(fake_pool):
    ...
    await entry_repo.contar_feed_admin(fake_pool)
...
@pytest.mark.asyncio
async def test_listar_por_usuario_junta_game_event_arena(fake_pool):
    fake_pool.set_fetch([{
        "id": "e1", "nick": "Campeao", "pontuacao": 5000, "foto_url": None,
        "no_ranking": True, "superado": False, "pendente": False, "arquivado": False,
        "criado_em": "2026-01-01",
        "game_id": "j1", "game_nome": "Pac-Man", "game_slug": "pac-man",
        "event_id": "ev1", "event_nome": "Canal3 Expo", "event_slug": "canal3expo",
        "arena_id": "m1", "arena_nome": "Canal3",
    }])

    resultado = await entry_repo.listar_por_usuario(fake_pool, "u1")

    assert len(resultado) == 1
    assert resultado[0]["arena_nome"] == "Canal3"
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
    assert "JOIN events ev" in sql
    assert "JOIN arenas m" in sql
...
# ── listar_ranking_por_events / listar_lideres_por_events ────────────────────
# Ranking agregado — docs/RANKINGS_CONFIGURAVEIS_SPEC.md §2.1, modos
# 'ultimo_evento'/'marca'/'marca_parceiras'.

@pytest.mark.asyncio
async def test_listar_ranking_por_events_filtra_por_lista_e_expoe_origem(fake_pool):
    fake_pool.set_fetch([{
        "id": "e1", "nick": "Campeao", "nome": None, "pontuacao": 5000,
        "foto_url": None, "event_id": "ev1", "user_id": None, "criado_em": "2026-01-01",
        "event_nome": "Canal3 Expo", "event_slug": "canal3expo", "arena_nome": "Canal3",
    }])

    resultado = await entry_repo.listar_ranking_por_events(fake_pool, "j1", ["ev1", "ev2"])

    assert len(resultado) == 1
    assert resultado[0]["arena_nome"] == "Canal3"
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
    assert "e.event_id = ANY($2::uuid[])" in sql
    assert "JOIN events ev ON ev.id = e.event_id" in sql
    assert "JOIN arenas m ON m.id = ev.arena_id" in sql
    assert "ORDER BY e.pontuacao DESC, e.criado_em ASC, e.id ASC" in sql
    assert fake_pool.fetch.call_args[0][1:] == ("j1", ["ev1", "ev2"])


@pytest.mark.asyncio
async def test_listar_ranking_por_events_vazio(fake_pool):
    fake_pool.set_fetch([])
    resultado = await entry_repo.listar_ranking_por_events(fake_pool, "j1", ["ev1"])
    assert resultado == []


@pytest.mark.asyncio
async def test_listar_lideres_por_events_agrega_top1_por_game(fake_pool):
    fake_pool.set_fetch([
        {"game_id": "j1", "slug": "pac-man", "nick": "CAMPEAO", "pontuacao": 99000},
    ])

    resultado = await entry_repo.listar_lideres_por_events(fake_pool, "ev-atual", ["ev1", "ev2"])
...
@pytest.mark.asyncio
async def test_listar_feed_admin_sem_filtros_so_event_ids(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, limit=50, offset=0, event_ids=None)

    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
    assert "WHERE ($3::uuid[] IS NULL OR e.event_id = ANY($3::uuid[]))" in sql
    assert "JOIN events ev ON ev.id = e.event_id" in sql
...
async def test_listar_feed_admin_status_visiveis(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, status="visiveis")
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
async def test_listar_feed_admin_status_ocultos(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, status="ocultos")
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
async def test_listar_feed_admin_filtro_data(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, data_de="2026-01-01", data_ate="2026-01-31")
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
@pytest.mark.asyncio
async def test_listar_feed_admin_filtro_game_id(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, game_id="j1")
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
    assert "e.game_id = $4" in sql
...
async def test_listar_feed_admin_filtro_sem_foto(fake_pool):
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, sem_foto=True)
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
async def test_listar_feed_admin_filtro_sem_identificacao(fake_pool):
    ...
    não um só)."""
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, sem_identificacao=True)
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
@pytest.mark.asyncio
async def test_listar_feed_admin_busca_pesquisa_nick_game_event(fake_pool):
    """Sem full-text search — ILIKE direto sobre os 3 campos já
    existentes no feed (decisão do item 4.4)."""
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(fake_pool, busca="novato")
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
...
async def test_listar_feed_admin_filtros_combinados_indices_sequenciais(fake_pool):
    ...
    fake_pool.set_fetch([])
    await entry_repo.listar_feed_admin(
        fake_pool, data_de="2026-01-01", game_id="j1", busca="novato",
    )
    sql = " ".join(fake_pool.fetch.call_args[0][0].split())
    assert "e.criado_em::date >= $4" in sql
    assert "e.game_id = $5" in sql
...
async def test_contar_feed_admin_aplica_mesmos_filtros_que_listar(fake_pool):
    ...
    fake_pool.set_fetchval(0)
    await entry_repo.contar_feed_admin(fake_pool, status="pendentes", game_id="j1", busca="x")

    sql = " ".join(fake_pool.fetchval.call_args[0][0].split())
    assert "e.pendente = true" in sql
    assert "e.game_id = $2" in sql
...
@pytest.mark.asyncio
async def test_listar_lideres_por_events_none_e_modo_geral_sem_filtro(fake_pool):
    """event_ids=None (modo 'geral') não filtra por event nenhum —
    a query decide isso via ($2::uuid[] IS NULL OR ...)."""
    fake_pool.set_fetch([])

    await entry_repo.listar_lideres_por_events(fake_pool, "ev-atual", None)
