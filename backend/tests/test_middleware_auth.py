...
# ── Caminho 2: sessão + admin_vinculo ────────────────────────────────────────

@pytest.mark.asyncio
async def test_sessao_sem_vinculo_retorna_401(client):
    """Pessoa logada normal (visitante), sem NENHUM admin_vinculo —
    ...
    with patch("auth.service.obter_usuario_da_sessao", AsyncMock(return_value=usuario)), \
         patch("repositories.admin_vinculo.listar_por_usuario", AsyncMock(return_value=[])):
...
async def test_sessao_com_vinculo_marca_funciona_escopado(client):
    ...
    client.cookies.set("canal3_session", "sessao-valida")
    with patch("auth.service.obter_usuario_da_sessao", AsyncMock(return_value=usuario)), \
         patch("repositories.admin_vinculo.listar_por_usuario", AsyncMock(return_value=[vinculo])), \
         patch("repositories.admin_vinculo.tem_acesso_evento", AsyncMock(return_value=True)), \
...
async def test_sessao_com_vinculo_super(client):
    ...
    with patch("auth.service.obter_usuario_da_sessao", AsyncMock(return_value=usuario)), \
         ...
         patch("repositories.entrada.listar_feed_admin", AsyncMock(return_value=[])), \
         patch("repositories.entrada.contar_feed_admin",  AsyncMock(return_value=0)) as contar_mock:
        # Nem precisa de evento_id na URL — é super via sessão também
        resp = await client.get(_rota_teste_protegida())
