...
async def test_admin_escopado_nao_pode_criar_vinculo(client):
    ...
    resp = await client.post("/api/admin/vinculos",
        json={"user_id": make_uuid(), "escopo": "super"})
    assert resp.status_code == 403
...
async def test_criar_vinculo_super(client):
    ...
    with patch("repositories.admin_vinculo.criar", AsyncMock(return_value=_vinculo(escopo="super", evento_id=None))):
        resp = await client.post("/api/admin/vinculos",
            json={"user_id": user_id, "escopo": "super"})
...
async def test_criar_vinculo_marca_sem_marca_id_retorna_422(client):
    ...
    resp = await client.post("/api/admin/vinculos",
        json={"user_id": make_uuid(), "escopo": "marca"})
...
async def test_criar_vinculo_evento_sem_evento_id_retorna_422(client):
    ...
    resp = await client.post("/api/admin/vinculos",
        json={"user_id": make_uuid(), "escopo": "evento"})
...
async def test_criar_vinculo_super_com_evento_id_retorna_422(client):
    ...
    resp = await client.post("/api/admin/vinculos",
        json={"user_id": make_uuid(), "escopo": "super", "evento_id": make_uuid()})
...
async def test_criar_vinculo_escopo_invalido_retorna_422(client):
    ...
    resp = await client.post("/api/admin/vinculos",
        json={"user_id": make_uuid(), "escopo": "raiz-do-sistema"})

    assert resp.status_code == 422


@pytest.mark.asyncio
async def test_criar_vinculo_usuario_ou_evento_inexistente_retorna_404(client):
    app.dependency_overrides[require_admin] = lambda: SUPER_CTX
    pool = MagicMock()
    app.dependency_overrides[get_pool] = lambda: pool

    with patch("repositories.admin_vinculo.criar",
               AsyncMock(side_effect=Exception("violates foreign key constraint"))):
        resp = await client.post("/api/admin/vinculos",
            json={"user_id": make_uuid(), "escopo": "evento", "evento_id": make_uuid()})
