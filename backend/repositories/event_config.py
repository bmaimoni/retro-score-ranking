...
async def listar(pool: Pool) -> dict:
    ...
    rows = await pool.fetch("SELECT chave, valor, descricao FROM event_config ORDER BY chave")
...
async def get_publico(pool: Pool) -> dict:
    ...
    rows = await pool.fetch(
        "SELECT chave, valor FROM event_config WHERE chave LIKE 'upload_%' OR chave LIKE 'event_%'"
    )
