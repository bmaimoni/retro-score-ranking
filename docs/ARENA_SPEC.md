...
| D.8 | Revisão de D.1 (2026-09-02): separação landing/ação | `index.html` deixa de ser a "home institucional" híbrida (proposta de valor + login + criar Arena + diretório inteiro, tudo na mesma tela) e vira **landing page de marketing pura**: proposta de valor, "como funciona", features reais (nada de prometer o que ainda não existe, ex. badges de rivalidade/chave de campeonato do §8, ainda sem spec própria), e uma vitrine de "eventos abertos agora" (top N, social proof/discovery gancho) — inspirada no padrão de landing tipo BeeMessage (hero → demonstração/prova → como funciona → features → CTA final), adaptada ao tom do produto. Todo o funcional (login Google/magic link, criar Arena, diretório completo de eventos abertos) migra pro que era `index.html` hoje, renomeado **`login.html`** (Bruno: "pra uma tela de login mesmo"). CTAs da landing (criar Arena, ver evento específico, "ver todos os eventos") apontam pra lá; jogar/enviar pontuação a partir de um card de evento continua indo direto pra `play.html?evento=slug`, sem passar pelo login (nunca exigiu conta). Motivo da mudança: a mistura escondia a proposta de valor atrás de um formulário, ruim pra quem chega frio; página de ação sozinha, sem contexto, também não vende o produto pra ninguém. Risco assumido e aceito: um passo a mais de clique entre a landing e criar uma Arena — aceitável porque hoje a aquisição é via convite/boca a boca (rede de amigos), não tráfego frio pago que exigiria landing e ação na mesma tela pra não perder conversão; reavaliar se/quando existir aquisição paga de verdade. `nav.js` ("Início") continua apontando pra `index.html` — é literalmente a home do site; usuário logado vê CTA adaptado ("Ir pro meu painel") em vez do formulário de criar Arena, sem reimplementar o widget de login na landing |
| D.9 | Rename por consequência: `login.html` antigo (callback de verificação de magic link) → `verify.html` | Nome ficou livre pro uso novo em D.8. Efeito colateral aceito: qualquer magic link **já enviado e não clicado** no momento do deploy quebra (aponta pro `login.html` antigo). Risco tratado como desprezível — TTL curto do token (`magic_link_ttl_minutes`) e nenhuma campanha de convite em massa disparada nas horas antes do deploy. `backend/auth/service.py` (`solicitar_magic_link`) atualizado pra gerar o link com `/verify.html`. Aproveitando a rodada (pedido explícito do Bruno: nomes de arquivo novos em inglês daqui pra frente, e ir corrigindo o que não for inglês conforme for sendo mexido) — não vira rodada de rename geral do frontend agora; `perfil.html`, `convite.html`, `telao.html` ficam pendentes, mesma iniciativa "rename retroativo" já registrada em `PLANO_IMPLEMENTACAO_2026.md` (Fase 7) |
